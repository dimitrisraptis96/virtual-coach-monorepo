  ...
  view = {
    ...
    resize: function(e, w, h) {
      ...
      if (pickingTexture !== null)
        pickingTexture.setSize(Math.floor(w * 0.5), Math.floor(h * 0.5));
    ...
    pick: function() {
      ...
      renderer.readRenderTargetPixels(
        pickingTexture,
        Math.floor(mouseBase.x * 0.5),
        Math.floor(vs.h * 0.5) - Math.floor(mouseBase.y * 0.5),
        1,
        1,
        pixelBuffer
      );
    ...
    addShadow: function(b) {
      if (b) {
        ...
        renderer.shadowMap.type = view.isMobile
    ...
    correctMorph: function(morphs, name, meshs) {
      for (var i = 0; i < morphs.length; i++) {
        ...
        meshs[name].geometry.morphAttributes.position[i].array =
          meshs[name + "_" + morphs[i]].geometry.attributes.position.array;
        meshs[name].geometry.morphAttributes.normal[i].array =
          meshs[name + "_" + morphs[i]].geometry.attributes.normal.array;
      }
    },
    ...
    initEnvScene: function(map) {
      ...
      ballCamera.renderTarget.texture.minFilter =
      ...
      ball = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ map: ballTexture, depthWrite: false })
      );
      ball.geometry.scale(-1, 1, 1);
      ballScene.add(ball);

      sky = new THREE.Mesh(
        new THREE.SphereGeometry(1000, 20, 12),
        ball.material
      );
