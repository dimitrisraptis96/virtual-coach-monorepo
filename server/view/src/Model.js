...
var Model = function (type, meshs, morph) {
  ...
  for (i = 0; i < lng; i++) {
    ...
    if (this.type !== "man") {
      if (name === "lCollar" || name === "rCollar")
        v = new THREE.Vector3(0.9, 1, 1);
      if (name === "lShldr" || name === "rShldr")
        v = new THREE.Vector3(0.96, 1, 1);
      if (name === "lForeArm" || name === "rForeArm")
        v = new THREE.Vector3(0.96, 1, 1);
      if (name === "lHand" || name === "rHand")
        v = new THREE.Vector3(0.95, 0.95, 0.95);
      if (name.substring(0, 2) === "lf" || name.substring(0, 2) === "rf")
        v = new THREE.Vector3(0.95, 1, 1);
...
Model.prototype = {
  ...
  addHelper: function (b) {
    ...
    this.center.geometry.applyMatrix(
  ...
  look: function () {
    ...
    m.lookAt(
      this.eyeTarget.position.clone().add(new THREE.Vector3(0, -1.4, 0)),
      this.eye_l.position,
      this.eye_l.up
    );
  ...
  setMaterial: function (name) {
    ...
    if (m.normalScale !== undefined)
      m.normalScale = new THREE.Vector2(set.muscles, set.muscles);
  ...
  setTextures: function (txt, debug) {
    ...
    if (m.map !== undefined)
      m.map = debug ? this.txt.UV_Grid_Sm : this.txt.avatar_c;
    ...
    if (m.normalMap !== undefined)
      m.normalMap =
        this.type === "man" ? this.txt.avatar_n_m : this.txt.avatar_n_w;
    if (m.lightMap !== undefined)
      m.lightMap =
        this.type === "man" ? this.txt.avatar_l_m : this.txt.avatar_l_w;
    ...
    if (m.map !== undefined)
      m.map = this.type === "man" ? this.txt.eye_m : this.txt.eye_w;
    if (m.envMap !== undefined) m.envMap = view.getEnvmap();
  ...
  updateSetting: function () {
    ...
    if (m.normalScale !== undefined)
      m.normalScale.set(set.muscles, set.muscles);
    if (m.aoMapIntensity !== undefined) m.aoMapIntensity = set.oamap;
    if (m.lightMapIntensity !== undefined) m.lightMapIntensity = set.lightmap;
    if (m.metalness !== undefined) m.metalness = set.metalness;
    if (m.roughness !== undefined) m.roughness = set.roughness;
